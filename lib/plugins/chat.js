...
function inject (bot, options) {
  ...
  const isBedrock = options.isBedrock ?? false
  ...
  // Bedrock chat handling
  if (isBedrock) {
    bot._client.on('text', (data) => {
      if (data.type === 'chat') {
        const username = data.source_name
        const message = data.message
        
        // Create a basic chat message object
        const msg = {
          toString: () => message,
          getText: () => message,
          extra: [{ text: message }]
        ...
        // Emit both the legacy format and the new message format
        bot.emit('chat', username, message, null, msg)
        bot.emit('message', msg, 'chat', username)
        bot.emit('messagestr', message, 'chat', msg, username)
      }
    })
  }

  // Java Edition chat handling
  ...
  function chatWithHeader (header, message) {
    ...
    if (isBedrock) {
      ...
      if (isCommand) {
        // Handle Bedrock commands
        bot._client.write('command_request', {
          command: message.slice(1), // Remove the leading '/'
          origin: {
            type: 'player',
            uuid: bot.uuid || '',
